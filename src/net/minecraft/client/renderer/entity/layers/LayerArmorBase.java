...
public abstract class LayerArmorBase<T extends ModelBase> implements LayerRenderer<EntityLivingBase> {
   ...
   private void renderArmorLayer(EntityLivingBase entityLivingBaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale, EntityEquipmentSlot slotIn) {
      if (!Main.instance.moduleManager.getModuleByClass(NoArmorRender.class).getState()) {
            ...
            if (itemarmor.getEquipmentSlot() == slotIn) {
               ...
               if (Reflector.ForgeHooksClient_getArmorTexture.exists()) {
                  if (ReflectorForge.armorHasOverlay(itemarmor, itemstack)) {
                     EnchantEffect enchantEffect = (EnchantEffect)Main.instance.moduleManager.getModuleByClass(EnchantEffect.class);
                     int j = itemarmor.getColor(itemstack);
                     f3 = (float)(j >> 16 & 255) / 255.0F;
                     f4 = (float)(j >> 8 & 255) / 255.0F;
                     float f5 = (float)(j & 255) / 255.0F;
                     float[] color = Color.getHSBColor(EnchantEffect.hue / 255.0F, 1.0F, 1.0F).getColorComponents((float[])null);
                     if (Main.instance.moduleManager.getModuleByClass(EnchantEffect.class).getState()) {
               ...
               case LEATHER:
                  ...
                  GlStateManager.color(this.colorR * f, this.colorG * f3, this.colorB * f4, this.alpha);
                  t.render(entityLivingBaseIn, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scale);
                  if (!Config.isCustomItems() || !CustomItems.bindCustomArmorTexture(itemstack, slotIn, "overlay")) {
