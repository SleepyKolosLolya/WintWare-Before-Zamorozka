...
public class CustomColors {
   ...
   private static Pair<CustomColormap[], Integer> parseLightmapsRgb() {
      ...
      if (ainteger.length <= 0) {
         return new ImmutablePair((CustomColormap[])null, 0);
      } else {
         int j1 = ainteger[0];
   ...
   private static int getSmoothColorMultiplier(IBlockState p_getSmoothColorMultiplier_0_, IBlockAccess p_getSmoothColorMultiplier_1_, BlockPos p_getSmoothColorMultiplier_2_, CustomColors.IColorizer p_getSmoothColorMultiplier_3_, BlockPosM p_getSmoothColorMultiplier_4_) {
      ...
      for(k1 = l - 1; k1 <= l + 1; ++k1) {
         for(l1 = j1 - 1; l1 <= j1 + 1; ++l1) {
            ...
            i2 = p_getSmoothColorMultiplier_3_.getColor(p_getSmoothColorMultiplier_0_, p_getSmoothColorMultiplier_1_, blockposm);
            i += i2 >> 16 & 255;
            j += i2 >> 8 & 255;
            k += i2 & 255;
         }
   ...
   public static int getFluidColor(IBlockAccess p_getFluidColor_0_, IBlockState p_getFluidColor_1_, BlockPos p_getFluidColor_2_, RenderEnv p_getFluidColor_3_) {
      ...
      if (customcolors$icolorizer == null) {
         ...
         return Config.isSmoothBiomes() && !((CustomColors.IColorizer)customcolors$icolorizer).isColorConstant() ? getSmoothColorMultiplier(p_getFluidColor_1_, p_getFluidColor_0_, p_getFluidColor_2_, (CustomColors.IColorizer)customcolors$icolorizer, p_getFluidColor_3_.getColorizerBlockPosM()) : ((CustomColors.IColorizer)customcolors$icolorizer).getColor(p_getFluidColor_1_, p_getFluidColor_0_, p_getFluidColor_2_);
      }
   }
   ...
   private static int getPotionId(String p_getPotionId_0_) {
      if (p_getPotionId_0_.equals("potion.water")) {
         ...
         } while(!potion.getName().equals(p_getPotionId_0_));
